document.addEventListener('DOMContentLoaded', function() {
    ...
    // --- Funktionen zur Personenauswahl ---
    ...
    function handleDayClick(event) {
        ...
        if (!selectedDates.start || (selectedDates.start && selectedDates.end)) {
            ...
            if (!isValidDateRange(selectedDates.start, selectedDates.end)) {
                alert("Der ausgewählte Zeitraum enthält nicht verfügbare oder vergangene Tage. Bitte wählen Sie einen anderen Zeitraum.");
                selectedDates = { start: null, end: null };
            }
        }
        renderCalendar(currentMonth, currentYear);
    ...
    function updateBookingSummary() {
        if (selectedDates.start) {
            selectedStartDateSpan.textContent = selectedDates.start.toLocaleDateString('de-DE'); // Use de-DE for German date format
            if (selectedDates.end) {
                selectedEndDateSpan.textContent = selectedDates.end.toLocaleDateString('de-DE');
                const diffTime = Math.abs(selectedDates.end.getTime() - selectedDates.start.getTime());
                const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
                stayDurationSpan.textContent = `${diffDays} Nächte`;
                calculateTotalPrice();
                proceedToDetailsBtn.disabled = false;
            } else {
                selectedEndDateSpan.textContent = '--';
                stayDurationSpan.textContent = '-- Nächte';
    ...
    function calculateTotalPrice() {
        ...
        if (selectedDates.start && selectedDates.end) {
            ...
            while (currentDate < selectedDates.end) {
                ...
                total += adjustedPrice;
                pricesHtml += `<div>${currentDate.toLocaleDateString('de-DE')}: <span>${adjustedPrice} €</span></div>`;
    ...
    // --- Weiter zum Formular für persönliche Daten ---
    proceedToDetailsBtn.addEventListener('click', () => {
        if (selectedDates.start && selectedDates.end && guestCount >= minGuests) {
            ...
            alert("Bitte wählen Sie einen gültigen Aufenthaltszeitraum und die Anzahl der Personen, bevor Sie fortfahren.");
        }
    });
    ...
    // --- Absenden des finalen Buchungsformulars ---
    bookingForm.addEventListener('submit', (event) => {
        ...
        const confirmationMessage = `Endgültige Buchungsanfrage-Bestätigung:\n\n` +
                                    `Personen: ${bookingDetails.guests}\n` +
                                    `Von: ${new Date(bookingDetails.startDate).toLocaleDateString('de-DE')}\n` +
                                    `Bis: ${new Date(bookingDetails.endDate).toLocaleDateString('de-DE')}\n` +
                                    `Dauer: ${bookingDetails.durationNights} Nächte\n` +
                                    `Gesamtpreis: ${bookingDetails.totalPrice} €\n\n` +
                                    `Ihre Daten:\n` +
                                    `Name: ${bookingDetails.firstName} ${bookingDetails.lastName}\n` +
                                    `Land: ${bookingDetails.country}\n` +
                                    `E-Mail: ${bookingDetails.email}\n` +
                                    `Telefon: ${bookingDetails.phone}\n\n` +
                                    `Klicken Sie auf OK, um die endgültige Anfrage zu senden.`;

        if (confirm(confirmationMessage)) {
            console.log("Buchungsdetails zum Senden:", bookingDetails);
            alert("Buchungsanfrage erfolgreich gesendet! Sie werden per E-Mail zur Bestätigung kontaktiert.");

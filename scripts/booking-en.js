document.addEventListener('DOMContentLoaded', function() {
    ...
    function handleDayClick(event) {
        ...
        if (!selectedDates.start || (selectedDates.start && selectedDates.end)) {
            ...
            if (!isValidDateRange(selectedDates.start, selectedDates.end)) {
                alert("The selected period includes unavailable or past days. Please select another period.");
                selectedDates = { start: null, end: null };
            }
        }
        renderCalendar(currentMonth, currentYear);
    ...
    function updateBookingSummary() {
        if (selectedDates.start) {
            selectedStartDateSpan.textContent = selectedDates.start.toLocaleDateString('en-US'); // Use en-US for English date format
            if (selectedDates.end) {
                selectedEndDateSpan.textContent = selectedDates.end.toLocaleDateString('en-US');
                const diffTime = Math.abs(selectedDates.end.getTime() - selectedDates.start.getTime());
                const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
                stayDurationSpan.textContent = `${diffDays} nights`;
                calculateTotalPrice();
                proceedToDetailsBtn.disabled = false;
            } else {
                selectedEndDateSpan.textContent = '--';
                stayDurationSpan.textContent = '-- nights';
    ...
    function calculateTotalPrice() {
        ...
        if (selectedDates.start && selectedDates.end) {
            ...
            while (currentDate < selectedDates.end) {
                ...
                total += adjustedPrice;
                pricesHtml += `<div>${currentDate.toLocaleDateString('en-US')}: <span>${adjustedPrice} €</span></div>`;
    ...
    // --- Handle Proceed to Personal Details Form ---
    proceedToDetailsBtn.addEventListener('click', () => {
        if (selectedDates.start && selectedDates.end && guestCount >= minGuests) {
            ...
            alert("Please select a valid stay period and number of guests before proceeding.");
        }
    });
    ...
    bookingForm.addEventListener('submit', (event) => {
        ...
        const confirmationMessage = `Final Booking Request Confirmation:\n\n` +
                                    `Guests: ${bookingDetails.guests}\n` +
                                    `From: ${new Date(bookingDetails.startDate).toLocaleDateString('en-US')}\n` +
                                    `To: ${new Date(bookingDetails.endDate).toLocaleDateString('en-US')}\n` +
                                    `Duration: ${bookingDetails.durationNights} nights\n` +
                                    ...
                                    `Click OK to submit the final request.`;

        if (confirm(confirmationMessage)) {
            console.log("Booking details to send:", bookingDetails);
            alert("Booking request submitted successfully! You will be contacted via email for confirmation.");
